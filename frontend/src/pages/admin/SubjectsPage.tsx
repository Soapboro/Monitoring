...
const CONTROL_FORMS = ['Экзамен', 'Зачёт', 'Зачёт с оценкой', 'Курсовая работа', 'Курсовой проект']
...
export default function SubjectsPage() {
  ...
  return (
    <div className="p-8 max-w-4xl">
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-slate-500 font-medium">Форма контроля</th>
              ...
              {filtered.map(s => (
                <tr key={s.id} className="hover:bg-slate-50">
                  <td className="px-6 py-3">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">{deptName(s.department_id)}</p>
                  </td>
                  <td className="px-4 py-3 text-slate-400 font-mono text-xs">{s.code ?? '—'}</td>
                  <td className="px-4 py-3 text-slate-500">{s.control_form ?? '—'}</td>
      ...
      {deleteId !== null && (
        <ConfirmDelete
          text="Удалить дисциплину? Это действие необратимо."
...
function SubjectModal({ mode, subject, departments, onClose, onSave }: {
  ...
  const [controlForm, setControlForm] = useState(subject?.control_form ?? '')
  ...
  const submit = async () => {
    ...
    try {
      const payload = {
        ...
        hours_total: hoursTotal ? parseInt(hoursTotal) : undefined,
        control_form: controlForm || undefined,
  ...
  return (
    <Overlay onClose={onClose}>
      <h2 className="text-lg font-semibold text-slate-800 mb-5">
        {mode === 'create' ? 'Новая дисциплина' : 'Редактировать дисциплину'}
      </h2>
      ...
      <Field label="Количество часов (необязательно)">
