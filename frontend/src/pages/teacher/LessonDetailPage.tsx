...
export default function LessonDetailPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [assign, students, attRecords, gradeRecords] = await Promise.all([
        client.get<TeachingAssignment>(`/teaching-assignments/${l.assignment_id}`).then(r => r.data),
        client.get<StudentProfile[]>('/students', { params: { group_id: undefined } }).then(r => r.data),
        client.get<AttendanceRecord[]>('/attendance', { params: { assignment_id: l.assignment_id } }).then(r => r.data),
        client.get<GradeRecord[]>('/grades', { params: { assignment_id: l.assignment_id, grade_type: 'current' } }).then(r => r.data),
      ])

      setAssignment(assign)

      const [subj, grp, groupStudents] = await Promise.all([
      ...
      const gradeByStudent = Object.fromEntries(
        gradeRecords.filter(g => g.date_recorded === date).map(g => [g.student_id, g])
      )
  ...
  const saveAll = async () => {
    ...
    setSaving(true)
    try {
      // Сохраняем посещаемость одним запросом
      const dirtyAtt = rows.filter(r => r.dirty)
      if (dirtyAtt.length > 0) {
        // Если у строк уже есть запись — обновляем, иначе bulk создаём
      ...
      await Promise.all(dirtyGrades.map(async r => {
        ...
        if (r.grade) {
          if (!r.gradeValue.trim() && !r.gradeComment.trim()) {
            await deleteGrade(r.grade.id)
          } else {
            await updateGrade(r.grade.id, { value: val, comment: r.gradeComment.trim() || undefined })
          }
        } else if (r.gradeValue.trim() || r.gradeComment.trim()) {
          await createGrade({
  ...
  const dateStr = new Date(lesson.starts_at).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' })
  ...
  return (
    <div className="p-8 max-w-5xl">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm text-slate-400">{timeStr}</span>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Группа: {group?.name ?? '—'} · {dateStr}
            </p>
          </div>
          <button
            onClick={() => setEditOpen(true)}
            className="text-sm text-slate-400 hover:text-blue-600 border border-slate-200 px-3 py-1.5 rounded-lg hover:border-blue-300 transition-colors"
          >
            Редактировать
          </button>
        </div>
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-base font-semibold text-slate-700">Журнал занятия</h2>
        <div className="flex items-center gap-3">
          <button onClick={() => markAll(true)} className="text-xs text-emerald-600 hover:underline">Все присутствуют</button>
          <button onClick={() => markAll(false)} className="text-xs text-red-500 hover:underline">Все отсутствуют</button>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-slate-500 font-medium">Оценка</th>
                <th className="text-left px-4 py-3 text-slate-500 font-medium">Комментарий</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {rows.map((row, i) => (
                <tr key={row.student.id} className={row.dirty || row.gradeDirty ? 'bg-blue-50/40' : 'hover:bg-slate-50'}>
                  <td className="px-6 py-3 text-slate-400">{i + 1}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-blue-100 flex items-center justify-center shrink-0">
                        <span className="text-xs font-semibold text-blue-600">{row.student.last_name[0]}</span>
                      </div>
                      <div>
                  ...
                  <td className="px-4 py-3 text-center">
                    <button
                      ...
                      className={`w-8 h-8 rounded-full flex items-center justify-center mx-auto transition-colors font-bold text-sm ${
                        row.isPresent
                          ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200'
                          : 'bg-red-100 text-red-600 hover:bg-red-200'
                      }`}
                      title={row.isPresent ? 'Присутствует (нажми для изменения)' : 'Отсутствует (нажми для изменения)'}
                    >
                      {row.isPresent ? '✓' : '✗'}
                    </button>
                  </td>
